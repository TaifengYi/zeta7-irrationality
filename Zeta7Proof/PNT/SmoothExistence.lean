...
lemma smooth_urysohn_support_Ioo {a b c d : ℝ} (h1 : a < b) (h3: c < d) :
  ...
  have := exists_contMDiff_zero_iff_one_iff_of_isClosed
    (I := modelWithCornersSelf ℝ ℝ) (n := (⊤ : ℕ∞)) (s := Set.Iic a ∪ Set.Ici d) (t := Set.Icc b c)
...
lemma Function.support_id : Function.support (fun x : ℝ ↦ x) = Iio 0 ∪ Ioi 0 := by
  ext x; simp only [Function.mem_support, ne_eq, Iio_union_Ioi, mem_compl_iff, mem_singleton_iff]
