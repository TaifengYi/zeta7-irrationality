...
noncomputable def scale (g : CS n E) (R : ℝ) : CS n E := by
  ...
  · refine ⟨fun x => funscale g R x, ?_, ?_⟩
    · exact g.h1.comp (contDiff_id.const_smul R⁻¹)
...
lemma deriv_scale {f : CS (n + 1) E} : (f.scale R).deriv = R⁻¹ • f.deriv.scale R := by
  ...
  · simp [deriv]
  · exact ((f.hasDerivAt (R⁻¹ • v)).scomp v (by convert (hasDerivAt_id' v).const_mul R⁻¹ using 1 <;> first | rfl | simp | (ext; simp [smul_eq_mul]))).deriv
...
lemma integrable_iteratedDeriv_Schwarz {f : 𝓢(ℝ, ℂ)} : Integrable (iteratedDeriv n f) := by
  ...
  | succ n ih =>
    have e : ⇑(SchwartzMap.derivCLM ℝ ℂ f) = _root_.deriv f := funext fun x => by simp
    simpa [iteratedDeriv_succ', e] using ih (f := SchwartzMap.derivCLM ℝ ℂ f)
...
theorem W21_approximation (f : W21) (g : trunc) :
  ...
  have dh R v : HasDerivAt (h R) (h' R v) v := by
    refine (CS.hasDerivAt_scale (g : CS 2 ℝ) R v |>.const_sub 1).congr_deriv ?_
    simp [h', CS.deriv_scale']
    left; rfl
  ...
  · let F R v := ‖h'' R v * f v + 2 * h' R v * f' v + h R v * f'' v‖
    ...
    have e4 : ∀ᵐ (a : ℝ), Tendsto (fun n ↦ F n a) atTop (𝓝 0) := by
      apply Eventually.of_forall ; intro v
      have evg' : g' =ᶠ[𝓝 0] 0 := by convert ← g.zero.deriv <;> first | rfl | simp
      have evg'' : g'' =ᶠ[𝓝 0] 0 := by convert ← evg'.deriv <;> first | rfl | simp
