...
`lake env lean palomar_local_check/DumpChallenge.lean`). Starting from the compared theorem names,
...
It writes one line per reached constant to `palomar_local_check/challenge_closure.txt`; the
...
run_cmd do
  ...
  IO.FS.writeFile "palomar_local_check/challenge_closure.txt" (String.intercalate "\n" sorted.toList ++ "\n")
  logInfo m!"CLOSURE Challenge: {sorted.size} constants"
  for r in roots do
    let axs ← Lean.collectAxioms r
    logInfo m!"AXIOMS Challenge {r}: {axs.toList}"
